...
export function Environment(name: string): BaseURL {
    return `https://${name}-np-jobboard-ssv2.encr.app`
}
...
/**
 * Client is an API client for the np-jobboard-ssv2 Encore application. 
 */
...
export namespace jobs {
    ...
    export class ServiceClient {
        ...
        public async Jobs(): Promise<JobsResponse> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callAPI("POST", `/jobs`)
            return await resp.json() as JobsResponse
        }

        public async SaveJob(idStr: string): Promise<void> {
            await this.baseClient.callAPI("POST", `/save/${encodeURIComponent(idStr)}`)
        }

        public async SavedJobs(): Promise<JobsResponse> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callAPI("GET", `/saved`)
            return await resp.json() as JobsResponse
        }

        public async UnsaveJob(id: string): Promise<void> {
            await this.baseClient.callAPI("POST", `/unsave/${encodeURIComponent(id)}`)
        }
    }
}
...
class BaseClient {
    ...
    constructor(baseURL: string, options: ClientOptions) {
        ...
        this.headers = {
            ...
            "User-Agent":   "np-jobboard-ssv2-Generated-TS-Client (Encore/v1.29.6)",
        }
